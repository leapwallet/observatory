...
export namespace BlockchainNodeUrlGetter {
  export class DefaultApi {
    ...
    getCosmosUrl(blockchain: CosmosBlockchain): string {
      ...
      if (this.urlIndices[blockchain] >= urlList.length) {
        // patch for error condition, when unhealthy nodes are removed from HealthyNodes using report(), it needs to update here as well and update the index accordingly
